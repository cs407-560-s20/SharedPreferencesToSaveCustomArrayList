...
class MainActivity : AppCompatActivity() {
    ...
    fun checkBoxClickButton(view: View) {
        ...
        if (view is CheckBox) {
            ...
            when (view) {
                ch_car -> if (checked) vehicleList.add("Car") else vehicleList.remove("Car")
                ch_truck -> if (checked) vehicleList.add("Truck") else vehicleList.remove("Truck")
                ch_bus -> if (checked)  vehicleList.add("Bus") else vehicleList.remove("Bus")
                ch_boat -> if (checked) vehicleList.add("Boat") else vehicleList.remove("Boat")
                ch_airplane -> if (checked)  vehicleList.add("Airplane") else vehicleList.remove("Airplane")
                ch_bicycle -> if (checked)  vehicleList.add("Bicycle") else vehicleList.remove("Bicycle")
            }
        }
    ...
    private fun clearCheckBoxes(){
        ...
        ch_car.isChecked = false
        ch_truck.isChecked = false
        ch_bus.isChecked = false
        ch_boat.isChecked = false
        ch_airplane.isChecked = false
        ch_bicycle.isChecked = false
    }
    ...
    private fun loadCheckBoxOptions(vehicle: String) {
        ...
        when (vehicle) {
            "Car"       -> ch_car.isChecked = true
            "Truck"     -> ch_truck.isChecked = true
            "Bus"       -> ch_bus.isChecked = true
            "Boat"      -> ch_boat.isChecked = true
            "Airplane"  -> ch_airplane.isChecked = true
            "Bicycle"   -> ch_bicycle.isChecked = true
        }
    }
